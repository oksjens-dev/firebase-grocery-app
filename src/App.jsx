...
const CategoryBadge = ({ categoryId }) => {
 ...
 // If it's a known category, use its style. If custom, use a generic indigo style.
...
export default function App() {
 ...
 const [activeTab, setActiveTab] = useState('shopping'); // recipes, shopping, history, analytics
  // Data State
 ...
 const [newRecipeIngredients, setNewRecipeIngredients] = useState(''); // Text area, line separated
 ...
 useEffect(() => {
   ...
   // We use 'public' data path so the couple can share the list if they use the same App ID url
 ...
 // Merge default categories with any custom ones found in existing items
 const allCategories = useMemo(() => {
   const customSet = new Set();
  
   // Find categories in items that aren't in defaults
   items.forEach(i => {
   ...
   // Also include the currently selected category if it's custom
 ...
 const addItem = async (name, category = 'other') => {
   ...
   try {
     ...
     setNewItemName('');
     // Don't reset category immediately so user can add multiple items to same cat
   } catch (e) {
 ...
 const addRecipe = async () => {
   ...
   const ingredientsList = newRecipeIngredients.split('\n').filter(line => line.trim().length > 0).map(line => ({
     name: line.trim(),
     isDone: false // Local state for checklist view
   }));
       ...
       <section className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
         ...
         <div className="flex gap-3 overflow-x-auto pb-2">
           {[0, 1, 2, 3].map(offset => {
              ...
              // Find meal for this day using improved date matching
              ...
              return (
                <div key={offset} className="min-w-[130px] bg-indigo-50 p-3 rounded-xl flex flex-col justify-between h-28 relative group">
                  <div className="flex justify-between items-start">
                    ...
                    {/* If meal exists, show delete button, otherwise show Add button */}
                    {plannedMeal ? (
                       <button
                         onClick={(e) => {
                           e.stopPropagation();
     ...
     <div className="max-w-md mx-auto min-h-screen bg-white shadow-2xl relative">
         ...
         <div className="bg-slate-50/95 backdrop-blur-2xl border-t border-slate-200 pb-safe transition-all">
           <div className="flex justify-around items-center h-20 px-2 pb-2">
             <button
               ...
               className="flex-1 flex flex-col items-center justify-center group outline-none"
             >
               ...
               <span className={`text-[10px] font-semibold mt-1 transition-all duration-300 ${activeTab === 'recipes' ? 'text-indigo-600' : 'text-slate-400'}`}>
                 Recipes
               </span>
             </button>
             ...
             >
               ...
               <span className={`text-[10px] font-semibold mt-1 transition-all duration-300 ${activeTab === 'shopping' ? 'text-indigo-600' : 'text-slate-400'}`}>
                 List
               </span>
             </button>
             ...
             >
               ...
               <span className={`text-[10px] font-semibold mt-1 transition-all duration-300 ${activeTab === 'history' ? 'text-indigo-600' : 'text-slate-400'}`}>
                 Recent
               </span>
             </button>
             ...
             >
               ...
               <span className={`text-[10px] font-semibold mt-1 transition-all duration-300 ${activeTab === 'analytics' ? 'text-indigo-600' : 'text-slate-400'}`}>
                 Plan
               </span>
             </button>
           </div>
       ...
       >
         <div className="space-y-4">
             ...
             {isCreatingCategory ? (
               <div className="flex gap-2 animate-in fade-in zoom-in-95">
                 ...
                 <button
                   ...
                   className="bg-gray-100 text-gray-600 px-3 py-2 rounded-lg text-sm font-medium"
                 >
                   Cancel
                 </button>
               </div>
               ...
               <div className="grid grid-cols-2 gap-2">
                 {allCategories.map(cat => (
                   <button
                     ...
                     className={`p-2 rounded-lg text-sm text-left transition-all ${newItemCategory === cat.id ? 'ring-2 ring-indigo-500 bg-gray-50' : 'bg-white border border-gray-200'}`}
                   >
                 ...
                 <button
                   ...
                   className="p-2 rounded-lg text-sm text-center border border-dashed border-gray-300 text-gray-500 hover:bg-gray-50 hover:text-indigo-600 transition-colors flex items-center justify-center gap-1"
                 >
           ...
           <button
             onClick={() => { addItem(newItemName, newItemCategory); setIsAddItemOpen(false); }}
             className="w-full bg-indigo-600 text-white p-3 rounded-xl font-bold hover:bg-indigo-700 transition-colors mt-2"
           >
             Add to List
           </button>
         </div>
       ...
       >
          <div className="space-y-6">
             ...
             {/* Action 1: Add to List Only - Full Width now */}
             <button
               onClick={() => {
                 addRecipeToShoppingList(selectedRecipe);
                 setSelectedRecipe(null);
               }}
               className="w-full bg-indigo-600 text-white p-4 rounded-xl font-bold hover:bg-indigo-700 transition-colors flex items-center justify-center gap-2 shadow-lg"
             >
            ...
            {recipes.length === 0 ? (
              ...
              recipes.map(recipe => (
                <button
                  key={recipe.id}
                  onClick={() => {
                    planMeal(recipe, planningDayOffset);
                    setIsSelectRecipeOpen(false);
                  }}
                  className="w-full text-left p-4 rounded-xl bg-gray-50 hover:bg-indigo-50 border border-transparent hover:border-indigo-200 transition-all group"
                >
